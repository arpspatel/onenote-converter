...
export function FaqSection() {
  ...
  const faqs: FaqItem[] = [
    {
      ...
      answer: (
        <div className="space-y-2 text-neutral-400 text-xs">
          ...
          <p>
            To activate a trial or purchased license (usually a <code className="bg-[#16161A] text-indigo-300 border border-indigo-500/10 px-1.5 py-0.5 rounded font-mono">.lic</code> file), place the file into your classpath and initialize it <strong>before</strong> loading the OneNote document:
          </p>
          <div className="bg-black/40 border border-neutral-800 text-neutral-200 rounded-lg p-3 font-mono text-[10px] space-y-1 select-all leading-normal">
            <div>import com.aspose.note.License;</div>
            <div className="text-neutral-550">// Initialize once on application startup</div>
    ...
    },
    {
      ...
      icon: Type,
      answer: (
        <div className="space-y-2 text-neutral-400 text-xs text-justify">
          <p>
          ...
          <div className="bg-black/40 border border-neutral-800 text-neutral-300 rounded-lg p-3 font-mono text-[10px] select-all leading-normal">
      ...
      answer: (
        <div className="space-y-2 text-neutral-400 text-xs">
          <p>
            ...
            The generated code provides an opt-in block using <code className="bg-[#16161A] text-indigo-300 border border-indigo-500/10 px-1.5 py-0.5 rounded font-mono">LoadOptions</code>. By simply passing the decryption key into <code className="bg-[#16161A] text-indigo-300 border border-indigo-500/10 px-1.5 py-0.5 rounded font-mono">loadOptions.setPassword("my_key")</code>, Aspose.Note parses and decrypts files automatically without throwing a security exception.
          </p>
        </div>
      ),
    },
  ...
  return (
    <div className="bg-[#0E0E10] rounded-xl border border-neutral-800 shadow-xl p-6 space-y-4">
      <div className="flex items-center space-x-2 border-b border-neutral-850 pb-3">
        ...
        <h3 className="font-heading font-bold text-white text-xs uppercase tracking-widest">Converting FAQ & Best Practices</h3>
      </div>
          ...
          return (
            <div
              ...
              className={`border rounded-lg transition-all ${
                IsOpen ? 'border-indigo-500/30 bg-indigo-600/5' : 'border-neutral-850 hover:bg-[#16161A]/40'
              }`}
            >
              <button
                id={`btn-faq-question-${index}`}
                onClick={() => setOpenIndex(IsOpen ? null : index)}
                className="w-full text-left px-4 py-3.5 flex items-center justify-between font-sans text-xs font-semibold text-neutral-300 hover:text-white transition-colors cursor-pointer"
              >
                <div className="flex items-center space-x-2.5">
                  <span className="p-1 rounded bg-[#16161A] text-neutral-400 group-hover:bg-[#202026]">
                    <FaqIcon className="h-3.5 w-3.5 text-neutral-300" />
                  </span>
              ...
              {IsOpen && (
                <div className="px-5 pb-4 pt-1 border-t border-dashed border-neutral-850 animate-slide-down">
                  {faq.answer}
                </div>
              )}
            </div>
